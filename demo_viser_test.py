...
parser.add_argument(
    "--image_folder", type=str, default="../data/new_office/frames", help="Path to folder containing images"
)
parser.add_argument("--use_point_map", action="store_true", default=False, help="Use point map instead of depth-based points")
...
parser.add_argument("--save_results", type=str, default="40_to_end_resorted_confs_frq30_conf_50.glb", help="Path to save unified results (.npz)")
...
def main():
    ...
    image_paths = new_image_paths[40:]
    print(f"Found {len(image_paths)} images")
    print(image_paths)
    ...
    pose_confs = np.mean(np.mean(predictions['world_points_conf'], axis=2), axis=1)
    print(np.mean(predictions['world_points_conf']))


    sorted_paths = [
    item for _, item in                  # keep only the item
    sorted(zip(pose_confs, image_paths),             # pair up → [(0.87,"img_001"), …]
           key=lambda pair: pair[0],     # sort on confidence
           reverse=False)                 # True = highest first
    ]

    print(sorted_paths)
    images = load_and_preprocess_images(sorted_paths).to(device)
    print(f"Preprocessed images shape: {images.shape}")
    ...
    print(np.mean(predictions['world_points_conf']))
    glbfile = os.path.join( args.save_results)
